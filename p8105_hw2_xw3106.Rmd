...
```
`df` is the combined datasets that contains `r nrow(df)` observations of `r ncol(df)`variables. the range of year is from 1947 to 2015. the data set contains key variables like `year`,`month`, `president``unemployment`. 
#Problem 2
```{r}
#Data Cleaning
